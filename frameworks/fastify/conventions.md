...
Register `@fastify/view` with Eta as a global plugin. Add a `reply.page()` decorator for the common partial-vs-full-page pattern.
...
Use this full pattern for all SSE endpoints. Steps: authenticate → enforce per-user and per-org limits → register → keepalive → max-age → cleanup on disconnect.

```typescript
// In-memory registries — MVP (single node)
// For multi-node: subscribe each node to Redis Pub/Sub and forward locally
const sseUserCounts = new Map<number, number>();
const sseOrgCounts  = new Map<number, number>();
const sseRegistry   = new Map<number, Set<FastifyReply>>();

const MAX_USER_CONNECTIONS = 10;
const MAX_AGE_MS = 3_600_000; // 1 hour — force client reconnect

fastify.get('/api/sse/orgs/:orgId', async (request, reply) => {
  // 1. Authenticate and resolve limits
  const { userId, orgId, orgConnectionLimit } = await validateOrgSession(request);

  // 2. Per-user limit (tabs / devices)
  const userCount = sseUserCounts.get(userId) ?? 0;
  if (userCount >= MAX_USER_CONNECTIONS)
    return reply.status(429).send({ error: 'too_many_tabs' });

  // 3. Per-org limit (from license.seats * 5 or Community cap)
  const orgCount = sseOrgCounts.get(orgId) ?? 0;
  if (orgCount >= orgConnectionLimit)
    return reply.status(429).send({ error: 'org_connection_limit' });

  // 4. Register connection
  sseUserCounts.set(userId, userCount + 1);
  sseOrgCounts.set(orgId, orgCount + 1);
  if (!sseRegistry.has(orgId)) sseRegistry.set(orgId, new Set());
  sseRegistry.get(orgId)!.add(reply);
  ...
  // 6. Max-age: close after 1 hour, client reconnects silently
  const maxAge = setTimeout(() => reply.raw.end(), MAX_AGE_MS);

  // 7. Keepalive: detect dead connections every 30s
  ...
  request.raw.on('close', () => {
    ...
    sseRegistry.get(orgId)?.delete(reply);
    clearTimeout(maxAge);
    clearInterval(ping);
  });
});

// Broadcast helper — called by EventBus subscriber, not by route handlers directly
export function broadcastToOrg(orgId: number, event: string, data: object): void {
  const payload = `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
  for (const reply of sseRegistry.get(orgId) ?? new Set()) {
    reply.raw.write(payload);
  }
}
```

### EventBus abstraction (scale-out ready)

Ingest handlers and settings handlers call `eventBus.publish()` — never `broadcastToOrg()` directly. This allows transparent replacement with Redis Pub/Sub for multi-node deployments.
...
// Factory: select implementation from EVENT_BUS env var
// 'memory' = in-process EventEmitter (single node)
// 'redis'  = Redis Pub/Sub via ioredis (multi-node)
```
...
Register globally with a generous default, then tighten per sensitive route.

```typescript
// src/app.ts — register once after global plugins
await app.register(import('@fastify/rate-limit'), {
  global:     true,
  max:        600,       // requests per user per window
...
```typescript
// Login — strict, keyed by IP
fastify.post('/auth/login', {
  config: { rateLimit: { max: 10, timeWindow: 60_000 } },
}, handler);

// CTRF ingest — per-token limit read from DB (default 120/hour; 0 = unlimited)
fastify.post('/api/v1/projects/:slug/runs', {
  config: {
    rateLimit: {
      keyGenerator: (req) => (req.headers['x-api-token'] as string) ?? req.ip,
      max: async (req) => {
        const token = await resolveProjectToken(req);
...
// SSE connect — prevent rapid-reconnect amplification
fastify.get('/api/sse/*', {
  config: { rateLimit: { max: 1, timeWindow: 2_000 } },
}, handler);
```

Standard limits by endpoint class:

| Endpoint class | max | timeWindow | Key |
|---|---|---|---|
| Login / forgot-password | 10 | 60 s | IP |
| CTRF ingest (`POST /runs`) | DB-configurable (default 120) | 1 hour | API token |
| Settings `PATCH` | 60 | 60 s | authenticated user |
| SSE connect | 1 | 2 s | authenticated user |
| General authenticated API | 600 | 60 s | authenticated user |
