...
Register MikroORM as a global Fastify plugin (see `fastify/conventions.md`). Fork `em` per request, never share it.
...
Use a `settings JSONB` column (mapped as `p.json()`) for preferences that are never used in `WHERE` clauses or indexes. Keep queryable data in typed columns.

```typescript
const OrgSchema = defineEntity({
  name: 'Organization',
  tableName: 'organizations',
  properties: {
    id:            p.integer().primary(),
    slug:          p.string(100),
    retentionDays: p.integer().default(90), // queryable → typed column
    settings:      p.json().default('{}'),  // preferences → JSONB
    updatedAt:     p.datetime().defaultRaw('now()').onUpdate(() => new Date()),
  },
});
```

**Rule:** `settings JSONB` if never queried/indexed. Typed column if used in `WHERE`, `JOIN`, or aggregate.

Atomic key merge in PostgreSQL (avoids overwriting other keys):

```typescript
await em.getConnection().execute(
  `UPDATE organizations SET settings = settings || $1 WHERE id = $2`,
  [JSON.stringify({ timezone: 'America/Los_Angeles' }), orgId],
);
```

SQLite (dev/test) — read-modify-write in the application layer:

```typescript
const org = await em.findOneOrFail(Organization, id);
org.settings = { ...org.settings, timezone: 'America/Los_Angeles' };
await em.flush();
```

---

## Chunked Bulk Insert (Event Loop Yielding)

For large inserts (CTRF test results, imports), batch at 500 rows and yield the event loop between batches to keep the server responsive.
...
async function bulkInsertResults(em: EntityManager, results: TestResultDto[]) {
  for (let i = 0; i < results.length; i += CHUNK_SIZE) {
    ...
    // Yield event loop — lets concurrent requests be served between chunks
...
Use this pattern for any insert of more than ~1,000 rows in a single request handler.
