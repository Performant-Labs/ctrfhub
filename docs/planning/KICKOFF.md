...
8. **Page testing follows the Three-Tier Verification Hierarchy** from `~/Sites/ai_guidance/frameworks/drupal/theming/verification-cookbook.md` (org-wide Performant Labs standard). Tier 1 Headless (curl + grep / supertest, 1–5s) → Tier 2 Structural ARIA (Playwright accessibility tree, 5–10s) → Tier 3 Visual screenshots (Playwright screenshots + pixel-diff, 60–90s). **Never run Tier 3 before Tier 2 is green.** The cookbook is written in Drupal/Canvas idiom; your job is to translate it to our Fastify + HTMX + Eta stack (Tier 1 commands become curl against Fastify or supertest + cheerio assertions; the ARIA and visual tiers run under Playwright). Encode this as its own skill, bind it into the Test-writer role, and enforce tier escalation in the `implementstory` workflow.
...
- `~/Sites/ai_guidance/frameworks/drupal/theming/verification-cookbook.md` — **the Three-Tier Verification Hierarchy** (mandatory read; the methodology applies to CTRFHub's stack even though the commands are Drupal-flavored)
- `~/Sites/ai_guidance/frameworks/drupal/theming/operational-guidance.md` — tier selection defaults + "Tier 2 first, Tier 3 last" escalation rule
...
- `page-verification-hierarchy.md` — **the Three-Tier Verification Hierarchy adapted for CTRFHub's stack.** Source: `~/Sites/ai_guidance/frameworks/drupal/theming/verification-cookbook.md`. Content requirements:
