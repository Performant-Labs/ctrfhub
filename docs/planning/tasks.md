...
**Depends on:** AUTH-001 (Better Auth API drives the admin/apikey seed), CI-001 (Dockerfile + GHCR + workflow), CI-002 (`compose.yml`), INFRA-005 (schema-generator at boot — no migrate step in Tugboat build)
**Skills required:** `better-auth-session-and-api-tokens.md` (admin + apikey seed), `mikroorm-dual-dialect.md` (PG dialect on Tugboat), `ctrf-ingest-validation.md` (ingest endpoint contract)
**Test tiers required:** integration (CI itself produces a green preview build)
**Page verification tiers:** T1 Headless (`/health` returns 200 on the preview URL; ingest endpoint returns `201 { runId }` to a real CTRF POST). T2.5 smoke (login as admin, verify dashboard loads) **deferred to AUTH-003 / DASH-002 — those stories add UI-verify against the live Tugboat preview when the UI lands**
**Critical test paths:** Each PR opens → Tugboat builds the preview from `compose.yml` → schema-generator creates the DB at app boot (no migrate step per INFRA-005) → seed script creates 1 admin + 1 org + 1 project + 1 API key (`apikey.metadata.projectId` set) → preview URL + API key exposed to CI as env vars → CI E2E job POSTs a real CTRF report to the preview's ingest endpoint with `x-api-token` and asserts `201 { runId }`, closing the dog-food loop. UI-verify of the seeded admin's login + the report rendering in the run list is **deferred to AUTH-003 / DASH-002**.
**Acceptance (phased — Phase 1 of CI-003; UI-verify deferred to AUTH-003 / DASH-002):** `.tugboat/config.yml` at repo root defines services (PG + app) and build steps (`docker compose up` → wait `/health` → run seed); first PR opened after this story merges produces a working preview at `pr-N.<tugboat-subdomain>.tugboatqa.com`; seed script creates admin + org + project + apikey idempotently (survives Tugboat refresh); seed exposes the preview URL and the API key as Tugboat env vars (e.g., `CTRFHUB_PREVIEW_URL`, `CTRFHUB_PREVIEW_API_KEY`); `.github/workflows/ci.yml`'s E2E dog-food step replaces CI-001's placeholder with a real POST to the preview, asserts `201 { runId }`, and drops its `continue-on-error: true` — the step gates merge from this point on. **Deferred to UI stories** (originally part of CI-003 acceptance, intentionally not in this phase): "preview's CTRFHub has the seeded admin and can be logged into" → AUTH-003's E2E. "report is visible in the preview's run list" → DASH-002's E2E.
