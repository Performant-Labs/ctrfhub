...
| ~~Two-factor authentication (2FA)~~ | **Deferred — see PL-010 in `parking-lot.md`.** Not in MVP; promoted when the criteria in PL-010 are met. |
| Active sessions | Table of sessions with device/IP/last seen + "Revoke" per row |
| Revoke all other sessions | Bulk action |

**DB:** Managed by Better Auth (sessions table). When 2FA lands, Better Auth's two-factor plugin provides `users.twoFactorEnabled`, encrypted TOTP secret, and backup-code tracking — no new CTRFHub tables needed (see PL-010).
...
**DB:** `user_notification_preferences` — full schema in `database-design.md` §4.24. Slack DM channel is scoped out of MVP (see PL-009 for the per-user Slack identity table) — the settings page hides the Slack DM column until PL-009 ships.
...
| Field | Notes |
|---|---|
| ~~Invite by email~~ | **Deferred — see PL-011 in `parking-lot.md`.** MVP creates admins via `/setup` wizard and CLI `bootstrap-admin --force` only. |
| ~~Role assignment~~ | **Deferred to PL-011.** Schema retains Admin \| Viewer (two roles; fine-grained in Roles [Business]) for forward compatibility. |
| ~~Remove member~~ | **Deferred to PL-011.** |
| Member table | MVP: lists admins with name, email, joined date, last active. No per-row role-change or remove actions until PL-011. |

**DB:** `organization_members` table — `(org_id, user_id, role, invited_at, joined_at)`. `invited_at` is unused in MVP (no invite flow) but retained for forward compatibility.
...
**DB:** `sso_configurations` — full schema in `database-design.md` §4.25. Business Edition only; the table exists on Community but is empty.
...
**DB:** `org_integrations` — full schema in `database-design.md` §4.26. Credential fields inside `config` JSONB are encrypted via the `APP_ENCRYPTION_KEY` pattern (DD-016 generalised per DD-027).
...
**DB:** `audit_logs` — full schema in `database-design.md` §4.27. Business Edition only (Community installs leave the table empty). Append-only; nightly retention cron purges per org setting.
...
**DB:** `licenses` — full schema in `database-design.md` §4.28. The spec stores the full signed `license_key` blob (not just a hash) because the app needs the signed payload intact to re-verify on startup and on license change.
