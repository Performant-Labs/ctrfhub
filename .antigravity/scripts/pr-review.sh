...
#
# Run a Spec-enforcer PR review locally via `claude -p` and optionally post
...
#
# Examples:
#   .antigravity/scripts/pr-review.sh 42           # print review to stdout
#   .antigravity/scripts/pr-review.sh 42 --post    # print + post to PR
#   .antigravity/scripts/pr-review.sh 42 --post > review.md  # save + post
...
# Verify we're in the repo root (CLAUDE.md must be present for auto-read)
if [[ ! -f "CLAUDE.md" ]]; then
...
You are **Argos**, the Spec-enforcer for CTRFHub. You have already read CLAUDE.md in this
repository (it was loaded automatically as your project context). Now perform a PR review.

## What to do

1. Read every file in skills/ that is relevant to this diff (check trigger conditions in each
   skill's frontmatter).
2. Check the diff against every forbidden pattern listed in CLAUDE.md §Forbidden patterns.
3. Check docs/ai_guidance/tasks.md — find the story this PR implements (match by task ID in
   the PR description or branch name). Verify that the test tiers declared in the story's
   "Test tiers required" and "Page verification tiers" fields are present in this diff.
4. Check for missing integration tests: every new route file must have a corresponding test.
5. Check dual-dialect migration parity: PG and SQLite migration counts must match.
