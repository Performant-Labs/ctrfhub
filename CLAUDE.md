...
| Codename | Model | Role |
|---|---|---|
| **Prometheus** | Claude Opus 4.6 | Feature-implementer / test-writer on Uranus. Single implementation agent (replaced Daedalus + Talos after migration from Mac). |
| **Argos** | Claude Opus 4.7 | Orchestrator — assigns tasks via `.argos/<taskId>/brief.md`, runs spec-audits, gates stories, opens PRs. Talks to Prometheus through André as the human relay. |
